...
PZsparse::PZsparse(TYPE center_inp, TYPE* coeff_inp, uint64_t degree_inp[][NUM_FACTORS * 6], int num_monomials) {
    center = center_inp;
...
PZsparse::PZsparse(TYPE center_inp, TYPE* coeff_inp, uint64_t degree_inp[][NUM_FACTORS * 6], int num_monomials, Interval independent_inp) {
    center = center_inp;
...
void PZsparse::print(bool independent_only) {
    if (independent_only) {
...
void vecPZsparse::print(bool centerOnly) {
    cout << "{\n";
...
matPZsparse::matPZsparse(const PZsparse& cosElt, const PZsparse& sinElt, const int axis) {
...
void matPZsparse::print(bool centerOnly) {
    cout << "{{\n";
    ...
    if (!centerOnly) {
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                if (elt[i * 3 + j] == nullptr) {
                    cout << "0 ";
                }
                else {
                    cout << elt[i * 3 + j]->center << ' ';
                }
            }
            cout << '\n';
        }
