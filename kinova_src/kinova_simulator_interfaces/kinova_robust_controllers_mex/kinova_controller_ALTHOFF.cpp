...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    double* Kp_input = (double*)mxGetData(prhs[1]);

    double* Ki_input = (double*)mxGetData(prhs[2]);

    double* maxError_input = (double*)mxGetData(prhs[3]);

    Eigen::Vector2d Kp;
    Kp[0] = Kp_input[0]; Kp[1] = Kp_input[1];

    Eigen::Vector2d Ki;
    Ki[0] = Ki_input[0]; Ki[1] = Ki_input[1];
    
    // create a robust controller
    RobustController c(Kr, Kp, Ki, *maxError_input);
