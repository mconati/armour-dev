...
void SimplifiedObstacles::initialize(const double* obstacles_inp, const int num_obstacles_inp) {
    ...
    if (obstacles != nullptr && num_obstacles > 0) {
        ...
        // obstacle data
        cudaMemcpyToSymbol(dev_obstacles_fast, obstacles, num_obstacles * (MAX_OBSTACLE_GENERATOR_NUM + 1) * 3 * sizeof(double));
        ...
        for (uint i = 0; i < COMB_NUM; i++) {
            ...
            if (b_id < BUFFER_OBSTACLE_GENERATOR_NUM_FAST - 1) {
                b_id++;
            }
            else {
                a_id++;
        ...
        cudaMemcpyToSymbol(dev_combA_fast, combA, COMB_NUM * sizeof(uint));
        cudaMemcpyToSymbol(dev_combB_fast, combB, COMB_NUM * sizeof(uint));

        cudaMalloc((void**)&dev_buffered_c, NUM_NODES_AT_ONE_TIME * MAX_OBSTACLE_NUM * sizeof(Eigen::Vector3d));
        cudaMalloc((void**)&dev_buffered_G, NUM_NODES_AT_ONE_TIME * MAX_OBSTACLE_NUM * sizeof(Eigen::Matrix<double, 3, BUFFER_OBSTACLE_GENERATOR_NUM_FAST>));
...
__global__ void bufferObstaclesKernel(const Eigen::Matrix<double, 3, LINK_FRS_GENERATOR_NUM>* link_independent_generators, 
	...
	buffered_c[node_id * num_obstacles + obs_id](p_id) = dev_obstacles_fast[obs_id * (MAX_OBSTACLE_GENERATOR_NUM + 1) * 3 + p_id];

	// copy obstacle generator to buffered obstacle generator
	for (uint i = 0; i < MAX_OBSTACLE_GENERATOR_NUM; i++) {
		buffered_G[node_id * num_obstacles + obs_id](p_id, i) = dev_obstacles_fast[(obs_id * (MAX_OBSTACLE_GENERATOR_NUM + 1) + i + 1) * 3 + p_id];
	}
...
__global__ void polytope_PH(Eigen::Vector3d* buffered_c, 
	...
	uint a_id = dev_combA_fast[p_id];
	uint b_id = dev_combB_fast[p_id];

	__shared__ double G[BUFFER_OBSTACLE_GENERATOR_NUM_FAST][3];
	__shared__ double c[3];

	if (p_id < BUFFER_OBSTACLE_GENERATOR_NUM_FAST) {
	...
	for (uint j = 0; j < BUFFER_OBSTACLE_GENERATOR_NUM_FAST; j++) {
