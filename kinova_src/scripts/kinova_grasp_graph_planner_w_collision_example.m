...
if random_start_goal
    ...
    %     % if valid, add to list of q's
        if abs(q_ee_euler_angles(1))<0.15 & abs(q_ee_euler_angles(2))<0.15 % & abs(q_ee_euler_angles(3))<0.03
            start = q_rand;
            findin% idx = (binsize(bin) == max(binsize));
% 
% Q_Graph_Reduced = subgraph(Q_Graph,idx);
% q_valid_list_reduced = q_valid_list(:,idx);g_start = false;
        end
    end
    finding_goal = true;
    while finding_goal
...
% Load Connected Configuration Graph

graph_struct = load('Connected_70k_w_Collision.mat');
Q_Graph_Reduced = graph_struct.Q_Graph_Reduced;

% Check Connectedness
[bin, binsize] = conncomp(Q_Graph_Reduced,'Type','weak');
if length(binsize) > 1
    fprintf('Graph May Not Be Fully Connected')
end

% Extract Attributes
Q_Attributes = table2array(Q_Graph_Reduced.Nodes);
Q_Nodes = Q_Attributes(:,1:7);
